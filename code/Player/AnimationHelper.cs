...
public sealed class AnimationHelper : Component
{
	...
	[Property] public GameObject IkHead { get; set; }
	...
	protected override void OnUpdate()
	{
		...
		Target.Set( "leftHandPos", IkLeftHand.Transform.Position );
		Target.Set( "rightHandPos", IkRightHand.Transform.Position );
		Target.Set( "headPos", IkHead.Transform.Position );
		Transform.LocalPosition = GameObject.Parent.Transform.World.PointToLocal(Head.Transform.Position.WithZ(0));
		LeftHand.Transform.Position = IkLeftHand.Transform.Position;
		LeftHand.Transform.Rotation = IkLeftHand.Transform.Rotation;
		RightHand.Transform.Rotation = IkLeftHand.Transform.Rotation;
		DuckLevel = 0.5f;// 1 - GetPercentageBetween(Transform.World.PointToLocal(Head.Transform.World.Position).z,38,55);
		//WithVelocity(0);
		Log.Info(Head.Transform.Position.z);
	...
	public static float GetPercentageBetween(float value, float min, float max)
	{
